...
RSpec.describe "Go daemon priority queues (integration)", :integration do
  ...
  before(:each) do
    ...
    @tmpdir = Dir.mktmpdir("kbatch-prio-#{suffix}")
    @marker_path = File.join(@tmpdir, "marker")
    @p0_topic = "kb.prio.p0.#{suffix}"
    @p1_topic = "kb.prio.p1.#{suffix}"
    @events_topic = "kb.prio.events.#{suffix}"
    @callbacks_topic = "kb.prio.callbacks.#{suffix}"
    @dlt_topic = "kb.prio.dlt.#{suffix}"
    @retry_base = "kb.prio.retry.#{suffix}"
  ...
  it "runs p1 jobs through the priority consumer group when p0 is idle" do
    job_id = KafkaBatch::Batch.enqueue_job("integration.go_p1", { "n" => 1 })
  ...
  it "processes p0 rank jobs on the higher-priority topic" do
    ...
    deadline = Time.now + 45
    until File.exist?(marker_p0) || Time.now >= deadline
      sleep 0.25
    end
    expect(File.read(marker_p0)).to eq(job_id)
  end
end
