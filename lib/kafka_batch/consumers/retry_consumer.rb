...
module KafkaBatch
  module Consumers
    ...
    class RetryConsumer < Karafka::BaseConsumer
      ...
      def consume
        messages.each { |msg| process_retry(msg) }
      end
      ...
      def process_retry(message)
        ...
        unless retry_after && retry_to
          KafkaBatch.logger.error(
            "[KafkaBatch][RetryConsumer] Malformed retry message (missing retry_after or retry_to) " \
            "– forwarding to DLT: #{data.inspect}"
          )
        ...
        wait_seconds = retry_after - Time.now
